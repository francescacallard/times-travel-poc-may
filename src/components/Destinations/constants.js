import greece from '../../assets/greece.png';
import france from '../../assets/france.png';
import italy from '../../assets/italy.png';
import louise from '../../assets/louise.png';

export const destinations = [
  { continent: 'Europe', country: 'Italy', image: italy },
  { continent: 'Europe', country: 'Greece', image: greece },
  { continent: 'Europe', country: 'France', image: france },
  { continent: 'South America', country: 'Brazil', image: greece },
  { continent: 'Africa', country: 'South Africa', image: greece },
];

export const journalists = [
  { name: 'Louise Roddon', title: 'Travel writer for The Times for 35 years', image: louise },
  { name: 'John Smith', title: 'Travel journalist for The Sunday Times', image: louise },
  { name: 'Emma Johnson', title: 'Travel columnist for The Times', image: louise },
  { name: 'David Brown', title: 'Senior travel editor at The Times', image: louise },
];

export const months = [
    'January', 'February', 'March', 'April', 'May', 'June',
    'July', 'August', 'September', 'October', 'November', 'December'
  ]